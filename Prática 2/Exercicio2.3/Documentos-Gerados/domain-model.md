...
> **Status:** ✅ Aprovado (v0.5) — aprovação confirmada pelo usuário em 2026-08-13, junto com a aprovação do `guardrails.md` v0.8 e do `agents-md-product-specialist.md`. Esta versão é uma **nova versão derivada** da v0.4 (✅ Aprovada, íntegra e inalterada em `Prática 2\Exercicio2.1\Documentos-Gerados\domain-model.md`), criada durante o Exercicio 2.3 a partir de um gap analysis entre a v0.4 e os termos de glossário citados no enunciado do Ex. 2.3.
> **Fontes usadas:** todas as da v0.4 (Cenário-Âncora 2, Anexo A, Anexo B, spec de requisitos de RAG resumida, dados de discovery) + o enunciado do Exercicio 2.3 (termos de glossário exemplo, usado como checklist de cobertura, não como substituto).
...
Termos de processo/metodologia (o que é "bounded context", "linguagem ubíqua" etc.) estão explicados em `dicionario-de-termos.md` (Exercicio 2.1). Aqui tratamos apenas do vocabulário de **negócio**.
...
| **Divisão de responsabilidade entre módulos** | **Criação e manutenção do metadado de vigência** (marcar qual versão está ativa/obsoleta) é responsabilidade do `pipeline-ingestao` (ADR-0003) — **fora do escopo desta subfase**. **Consumo do metadado de vigência** é responsabilidade do `query-endpoint` — mas apenas para **uso interno** (ex: ranking de recuperação); **não decide o que é exibido**: diante de qualquer contradição de fonte detectada, o assistente sempre mostra ambas as versões, independente da vigência estar definida ou não (decisão registrada no `requirements.md` §4, reafirmada no Exercicio 2.3 — ver nota abaixo). |
...
> **Nota de reafirmação (Ex. 2.3):** durante a construção da seção "Product Rules & Guardrails" do AGENTS.md, um enunciado simulado de guardrails sugeria "priorizar a versão mais recente e informar que existe versão anterior" como comportamento de **exibição**. Isso foi conscientemente **não incorporado** como regra de exibição — reproduziria o erro do Incidente 2 (citar a versão errada) e o conselho informal do FAQ item 8 do Anexo A, já tratado como armadilha conhecida. A priorização por vigência (ADR-0003) permanece restrita ao **ranking interno de recuperação**; a exibição continua sempre mostrando ambas as versões, sem exceção (`GR-Q-02`, `guardrails.md`).
...
| **Multiplicador regional** *(novo, v0.5)* | Fator multiplicador aplicado no cálculo de frete especial conforme a região de destino da carga. **Atenção para agentes de IA:** os valores divergem entre **PROC-042 v1** (ex: Sul 1,2) e **PROC-042 v2** (ex: Sul 1,3) — as duas tabelas coexistem sem vigência formal definida (mesma contradição do Incidente 2). Nunca combinar o multiplicador de uma versão com o fator de peso de outra numa mesma resposta (ver `guardrails.md`, GR-N-03). | Frete Especial / Governança Documental | PROC-042 §2.1 (v1 e v2) |
...
| **Tempo de resolução (SLA)** *(novo, v0.5)* | Tempo entre a abertura do chamado e a **efetiva resolução** do problema — distinto do "tempo de primeira resposta" (retorno inicial ao cliente, mesmo que só confirmando que está sendo verificado). Varia por tier: **Gold** até 24h úteis (chamados gerais) / até 4h (incidente crítico); **Silver** até 48h úteis / até 8h; **Standard** até 72h úteis / até 24h. **Atenção para agentes de IA:** nunca confundir "tempo de resposta" com "tempo de resolução" — são duas métricas diferentes na mesma tabela SLA-2024, com valores diferentes por tier (ver também FAQ item 41 do Anexo A, que descreve a confusão como dúvida recorrente de atendente). | SLA e Clientes | SLA-2024 §2 |
...
| **Vigência** | Metadado que indica se um documento (ou uma de suas versões) está formalmente ativo. **Situação atual conhecida:** PROC-042 v1 e v2 coexistem sem vigência formal definida — nenhuma delas está marcada como obsoleta. **Uso restrito ao ranking interno de recuperação** — nunca decide o que é exibido ao atendente (ver nota de reafirmação, seção 2.5). | Governança Documental | Anexo A, ADR-0003 |
...
| **Pergunta multi-domínio** | Pergunta do atendente que cruza mais de um bounded context de conhecimento (ex: devolução + frete especial). Ocorre em ~15% dos chamados, segundo dados de discovery. | Consulta e Resposta do Assistente | Dados de discovery |
| **Fonte citada** | Toda resposta do assistente deve indicar explicitamente de qual documento (e, se aplicável, qual versão) a informação veio. Requisito não negociável (spec de RAG resumida). | Consulta e Resposta do Assistente | Spec de RAG resumida |
...
- **Idioma:** o assistente sempre responde em **português** — mesmo idioma da documentação-fonte e do atendente.
- **Perguntas de acompanhamento (multi-turn):** o assistente considera o histórico da conversa (limitado a 3 turnos, ADR-0002) para resolver referências implícitas — ex: atendente pergunta "e para o Nordeste?" logo após perguntar sobre frete para o Sul. A pergunta de acompanhamento permanece no mesmo bounded context da pergunta anterior, a menos que o novo turno mude claramente de assunto.
...
> **Nota de revisão:** a decisão de não usar o FAQ informal como fonte de resposta (nem com aviso) foi tomada de forma conservadora, para evitar o risco descrito no Anexo B ("armadilha" de responder pergunta crítica com fonte informal e confiança alta). Isso é **explicitamente revisável**.
...
| **Carga danificada em trânsito** | Só existe no FAQ informal (item 38); sem POL/PROC formal. | O FAQ não é usado como fonte de resposta — assistente informa que não há fonte formal para o tema. |
...
Quando uma pergunta cruza dois contextos (ex: "prazo de devolução de uma carga com frete especial"), o assistente deve recuperar e citar fontes de **cada** contexto envolvido — não deve fundir os dois num resumo sem atribuição clara de qual parte vem de qual fonte.

---

## 5. Decisões de modelagem — status pós checkpoint de Tech Lead (Passo 4, herdado da v0.4)

1. ✅ **Consulta e Resposta como bounded context próprio** — validado.
2. ✅ **Governança Documental como bounded context transversal separado** — validado, responsabilidade dividida entre `pipeline-ingestao` e `query-endpoint`.
3. ✅ **Gaps tratados como fronteira explícita, não como bounded context vazio** — validado.
4. ✅ **FAQ informal não é usado como fonte de resposta** — decisão conservadora, explicitamente revisável.
5. ✅ **Nível de confiança padronizado em Alta/Média/Baixa**.
6. ✅ **Idioma (português) e comportamento multi-turn** explicitados.
7. 🔄 *(novo, v0.5, pendente de validação)* **"Multiplicador regional" e "Tempo de resolução (SLA)" adicionados como entradas próprias na linguagem ubíqua** — gap identificado no Exercicio 2.3 ao comparar os termos citados no enunciado com a tabela da seção 3.
...
|---|---|---|---|
| 0.1 – 0.4 | 2026-08-11 | Product Specialist (Claude) + Arlindo | Ver histórico completo em `Prática 2\Exercicio2.1\Documentos-Gerados\domain-model.md` — recorte de domínio completo, ✅ Aprovado. |
| 0.5 | 2026-08-13 | Product Specialist (Claude) + Arlindo | **Nova versão criada no Exercicio 2.3**, em arquivo próprio (`Prática 2\Exercicio2.3\Documentos-Gerados\domain-model.md`), sem alterar a v0.4 original. Gap analysis entre a v0.4 e os termos de glossário citados no enunciado do Ex. 2.3. Adicionadas 2 entradas na linguagem ubíqua (§3): **"Multiplicador regional"** e **"Tempo de resolução (SLA)"** — ambas já referenciadas indiretamente em `guardrails.md` (GR-N-03, GR-N-06) mas sem definição própria até então. Adicionada nota de reafirmação em §2.5 sobre a distinção entre priorização por vigência no retrieval interno (ADR-0003) e a regra de exibição (sempre ambas as versões), motivada pelo mesmo gap analysis. Status: rascunho, pendente de validação do usuário. |
